...
piper = TTS(language='en') 

responses = {
    "greeting": ["Hello there!", "Hi!", "What can I do for you?"],
    "searching": ["Let me see...", "Searching...", "Looking for that information."],
    "found_it": ["Here's what I found:", "I have some results:", "This might be helpful:"],
    "not_found": ["I couldn't find anything.", "No results for that query.", "Sorry, I'm drawing a blank."],
    "performing_action": ["Sure, I can do that.", "No problem!", "On it!"],
    "action_complete": ["Done!", "There you go!", "Completed."],
    "asking_confirmation": ["Is that correct?", "Do you want me to proceed?", "Should I go ahead?"],
    "positive_confirmation": ["Okay, I will.", "Got it!", "Right away."],
    "negative_confirmation": ["Okay, I won't.", "No problem.", "Understood."],
    "error": ["Oops, something went wrong.", "I'm having trouble with that.", "I couldn't quite get that."],
}
...
def process_command(command):
    ...
    if any(keyword in command.lower() for keyword in ["memory", "process", "using more than"]):
        try:
            threshold = int("".join([char for char in command if char.isdigit()]))
            processes = get_memory_intensive_processes(threshold)
            if processes:
                speak("Here are the memory-intensive processes:")
                for process in processes:
                    speak(process)
            else:
                speak(f"No processes found using more than {threshold} MB of memory.")
        except ValueError:
            speak("Please specify a valid memory threshold in MB, like 'processes using more than 500 MB'.")
        ...
        if "and navigate to" in process_name:
            ...
            speak(f"I've started {process_name} and navigated to {url}.")
        else:
            start_process(process_name)
            speak(f"I've started {process_name}.")

    elif "close" in command.lower():
        process_name = command.split("close", 1)[1].strip()
        close_processes(process_name)
        speak(f"I've closed {process_name}.")

    elif any(keyword in command.lower() for keyword in ["monitor", "cpu", "of"]):
        ...
        try:
            ...
            speak(f"Monitoring CPU usage of '{process_name}'. I'll let you know if it exceeds {threshold}%.")
        except ValueError:
            speak("Please specify a valid CPU threshold percentage, like 'exceeds 80%'.")

    elif any(keyword in command.lower() for keyword in ["list", "background", "processes"]):
        processes = list_background_processes()
        if processes:
            speak("Here are the background processes:")
            for process in processes:
                speak(process)
        else:
            speak("No background processes found.")

    elif any(keyword in command.lower() for keyword in ["restart", "service"]):
        service_name = command.split("service", 1)[1].strip()
        restart_service(service_name)
        speak(f"I've restarted the {service_name} service.")
...
def main_interaction_loop():
    ...
    while True:
        print("\nOptions:")
        print("1. Speak your command")
        print("2. Type your command")
        print("3. Exit")
