...
print(f"Attempting to create/access ChromaDB at: {db_dir}")
...
def query_interface():
    while True:
        ...
        print("1. Query stored screenshots (text input)")
        print("2. Query stored screenshots (voice input)")
...
if __name__ == "__main__":
    print(f"Script running from directory: {current_dir}")
    
    # Check if the database directory exists
    if os.path.exists(db_dir):
        print(f"ChromaDB directory found at: {db_dir}")
    else:
        print(f"ChromaDB directory not found. It will be created at: {db_dir}")
