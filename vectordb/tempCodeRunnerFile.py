...
def capture_screenshots():
    while True:
        x, y = window.winfo_rootx(), window.winfo_rooty()  # Get the Tkinter window's position
        width, height = window.winfo_width(), window.winfo_height()  # Get the window's size

        # Screenshot the area below the window
        screenshot_region = (x, y + height, x + width, y + height + 500)  # Adjust '500' as needed
